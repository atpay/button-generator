...
describe "atpay-button-generator" do
  ...
  let(:cli_input) { "bin/atpay-button-generator --amount 5.0 -p #{private_key} -u #{public_key} -r #{options[:partner_id]} -s subject -i imageurl -c color -t title -w false -e sandbox -n #{file_info}" }
  ...
  describe "button creation" do
    ...
    it "generates valid usage information" do
      assert_match(/--help, -h:   Show this message/,`bin/atpay-button-generator -h`)
    end
