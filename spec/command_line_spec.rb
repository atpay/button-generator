...
describe "atpay-button-generator" do
  ...
  let(:cli_input) { "atpay-button-generator --amount 5.0 -p #{private_key} -u #{public_key} -r #{options[:partner_id]} -s subject -i imageurl -c color -t title -w false -e sandbox -n #{file_info}" }
  ...
  describe "button creation" do
    ...
    it "contains button identifier text" do
      assert_match(/BUTTON CODE/,cli_output)
    end

    it "generates valid usage information" do
      assert_match(/--help, -h:   Show this message/,`atpay-button-generator -h`)
    end
